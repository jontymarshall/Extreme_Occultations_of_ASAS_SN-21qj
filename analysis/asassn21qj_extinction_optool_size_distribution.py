...
direc = '/Users/jonty/mydata/asassn21qj/size_distribution/'
...
#range of grain sizes for power law distributions
...
sizes = np.linspace(0.01,1.01,101)
...
for composition in compositions:
    ...
    for size in sizes:
        ...
        os.system("optool "+composition+" 1.0 -a "+str(size)+" 1000 3.5 -l 0.2 2.0 -fmax 0")
        ...
        csq = np.sum((rmi - y_ri_fit)**2/urmi**2) + np.sum((rmz[good] - y_rz_fit[good])**2/urmz[good]**2) / (2*len(gmr) - 6)
    ...
    filename="asassn21qj_size_dist_"+composition_neat+".json"
    ...
    plt.tight_layout()
    fig.savefig(direc + 'asassn21qj_lcogt_gr-ri_plot_'+composition+'size_dist_all.pdf',dpi=200)
    #plt.show()
    plt.close()
    plt.clf()
    ...
    ax1.legend(loc='lower right')
    plt.tight_layout()
    fig.savefig(direc + 'asassn21qj_lcogt_gr-rz_plot_'+composition+'size_dist_all.pdf',dpi=200)
    #plt.show()
    plt.close()
    plt.clf()
    ...
    if len(output_sizes) > 0:
        ...
        ax1.legend(loc='lower right')
        plt.tight_layout()
        fig.savefig(direc + 'asassn21qj_lcogt_gr-ri_plot_'+composition+'_size_dist.pdf',dpi=200)
        #plt.show()
        plt.close()
        plt.clf()
        ...
        ax1.legend(loc='lower right')
        plt.tight_layout()
        fig.savefig(direc + 'asassn21qj_lcogt_gr-rz_plot_'+composition+'_size_dist.pdf',dpi=200)
        #plt.show()
        plt.close()
        plt.clf()
